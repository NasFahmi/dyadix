class SystemPrompt:
    ...
    def get_system_prompt_decision(self) -> str:
        ...
        return (
            ...
            "- Microstructure: real-time metrics (CVD, orderbook imbalance, whale execution clusters, liquidations, and transaction velocity)\n"
            ...
            "- ALWAYS place your Stop Loss (SL) and Target (TP) using the values from 'structure_map' in your context. "
            "These are pre-calculated structural levels (Order Blocks, Liquidity Pools, PDH/PDL). "
            "For BUY: SL is below a Bullish OB or Low Pool; TP is at the nearest Strong pool, PDH, or Bearish OB above entry. "
            "For SELL: SL is above a Bearish OB or High Pool; TP is at the nearest Strong pool, PDL, or Bullish OB below entry.\n"
            "- Your 'stop_loss' and 'target' output MUST match 'structure_map.recommended_sl' and 'structure_map.recommended_tp' from your context. "
            "Do NOT invent new SL/TP levels. If structure_map is empty or has should_wait=true, output WAIT.\n"
            "- CRITICAL: Calculate the Risk/Reward (R:R) ratio using the values in structure_map: "
            "R:R = structure_map.natural_rr. The minimum acceptable R:R is 1.5. "
            "If structure_map.natural_rr < 1.5, you MUST output WAIT.\n"
            "- In your 'rr_calculation', show: Step 1: Entry = realtime_price. Step 2: Structural SL = structure_map.recommended_sl "
            "(type: structure_map.sl_type). Step 3: Structural TP = structure_map.recommended_tp (type: structure_map.tp_type). "
            "Step 4: R:R = structure_map.natural_rr. >= 1.5? Yes/No.\n"
            "- 'invalidated_if' MUST reference the structural level: e.g., 'price closes above Bearish OB top at 62117' or "
            "'price closes below Bullish OB bottom at 61800'. Use structure_map.sl_type and structure_map.sl_raw_level.\n"
            "- If market structure between entry and TP (structure_map.recommended_tp) shows a major opposing level, note it in key_risks.\n"
